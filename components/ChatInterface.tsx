...
export function ChatInterface() {
  const [threads, setThreads] = React.useState<Thread[]>([
    { id: crypto.randomUUID(), title: "Greeting Title", messages: [] },
  ]);
  ...
  const [selectedCategory, setSelectedCategory] =
    React.useState<Category>("create");
  ...
  React.useEffect(() => {
    try {
      if (activeThreadId)
        localStorage.setItem(STORAGE_KEYS.ACTIVE, activeThreadId);
    } catch {}
  }, [activeThreadId]);
  ...
  const showWelcome =
    (activeThread?.messages.length ?? 0) === 0 && input.trim().length === 0;
